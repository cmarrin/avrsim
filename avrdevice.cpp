...
void AvrDevice::Load(const char* fname) {
    ...
    //reading out the symbols
    {
        ...
        for(i = 0; i < number_of_symbols; i++) {
            ...
            if(!symbol_table[i]->section)
                continue;
            unsigned int lma = (int) symbol_table[i]->section->lma;
            unsigned int vma = (int) symbol_table[i]->section->vma;
            ...
            if(vma < 0x7fffff) { //range of flash space
                ...
                {
                    pair<unsigned int, string> p((unsigned int) symbol_table[i]->value + offset, symbol_table[i]->name);
                    //symbolsData.insert(p);
                    data->AddSymbol(p);  //not a real data container, only holding symbols!

                    pair<unsigned int, string> pp((unsigned int) symbol_table[i]->value + lma, symbol_table[i]->name);
                    //symbols.insert(pp);
                    Flash->AddSymbol(pp);
                }
            }
    ...
    while(sec != 0)  { 
        if(sec->flags & SEC_LOAD && sec->vma < 0x80ffff) { //only read flash bytes and data
            ...
            Flash->WriteMem(tmp, (unsigned int) sec->lma, size);
            free(tmp);
        }
        ...
        if(sec->flags & SEC_LOAD && sec->vma >= 0x810000) {
            ...
            unsigned char *tmp = (unsigned char *)malloc(size);
            bfd_get_section_contents(abfd, sec, tmp, 0, size);
            unsigned int offset = (unsigned int) sec->vma - 0x810000;
...
int AvrDevice::Step(bool &untilCoreStepFinished, SystemClockOffset *nextStepIn_ns) {
    ...
    if(trace_on == 1) {
        ...
        traceOut << sym << " ";
        for (int len = (int) sym.length(); len < 30;len++)
            traceOut << " " ;
    }
